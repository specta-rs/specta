...
/// The error type for the Valibot exporter.
#[non_exhaustive]
pub struct Error {
    kind: ErrorKind,
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match &self.kind {
            ...
            ErrorKind::BigIntForbidden { path } => write!(
                f,
                "Attempted to export {path:?} but Specta forbids exporting BigInt-style types (usize, isize, i64, u64, i128, u128, f128) to avoid precision loss. Remap them to a Valibot schema such as `specta_valibot::define(\"v.bigint()\")` to override this."
            ),
            ...
            ErrorKind::UnsupportedOpaqueReference(reference) => write!(
                f,
                "Found unsupported opaque reference '{}'. It is not supported by the Valibot exporter.",
                reference.type_name()
            ),
            ...
            ErrorKind::UnableToExport(layout) => {
                write!(
                    f,
                    "Unable to export layout {layout} with `Valibot::export`. Use `Valibot::export_to` or change layout."
                )
            }
        }
    }
