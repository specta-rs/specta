...
const GENERATED_FILE_BINDINGS: &[&str] = &[
    "v",
    "$spectaIsRecord",
    "$spectaEncodeKey",
    "$spectaDecodeKey",
    ...
    pub fn export_to(
        ...
        fn export(
            ...
            let module_types = module
                ...
                .collect::<Vec<_>>();
            let local_type_names = module_types
                .iter()
...

        {
            ...
            if !runtime.is_empty() || should_export_user_types {
                files.insert(runtime_path, {
                    ...
                    let runtime_import_paths = runtime_references
                        .into_iter()
                        .filter_map(|r| {
                            types
                                .get(&r)
                    ...
                    let existing_import_paths = types
                        .into_unsorted_iter()
                        ...
                        .map(str::to_string)
                        .collect::<BTreeSet<_>>();
                    let all_import_paths = runtime_import_paths
                        .union(&existing_import_paths)
                        .cloned()
                        .collect::<BTreeSet<_>>();
                    let import_paths = runtime_import_paths
                        .into_iter()
                        .filter(|module_path| !existing_import_paths.contains(module_path))
                        .collect::<BTreeSet<_>>();

                    let local_type_names = types
                        .into_unsorted_iter()
                        .filter(|ndt| {
                            ndt.ty.is_some()
...
fn validate_file_bindings<'a>(
    ...
    for module_path in import_paths {
        ...
        if !file_bindings.insert(alias.clone()) {
            return Err(Error::duplicate_file_binding(alias));
        }
    }
