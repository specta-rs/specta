...
// and `[]` are rejected). Specta preserves the declared field type so the
// exporter can retain that bare wire shape.
#[derive(Type, Serialize, Deserialize)]
#[specta(collect = false)]
...
#[test]
fn newtype_struct_skip_exports_bare_inner_type() {
    let unified = Typescript::default()
        .export(
            &Types::default().register::<NewtypeStructSkip>(),
            specta_serde::Format,
        )
        .expect("the ignored skip should retain the bare inner type");
    assert!(unified.contains("type NewtypeStructSkip = number"));

    let phases = Typescript::default()
        .export(
            &Types::default().register::<NewtypeStructSkip>(),
            specta_serde::PhasesFormat,
        )
