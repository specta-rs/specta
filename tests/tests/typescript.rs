...
use specta::{
    datatype::{DataType, Enum, EnumVariant, Field, NamedDataTypeBuilder, Primitive, Reference},
    Type, TypeCollection,
};
...
#[test]
fn runtime_built_unnamed_enum_variants_preserve_field_types() {
    let mut types = TypeCollection::default();

    let mut enm = Enum::new();
    enm.variants_mut().push((
        "A".into(),
        EnumVariant::unnamed()
            .field(Field::new(DataType::Primitive(Primitive::bool)))
            .build(),
    ));
    enm.variants_mut().push((
        "B".into(),
        EnumVariant::unnamed()
            .field(Field::new(DataType::Primitive(Primitive::i32)))
            .build(),
    ));

    NamedDataTypeBuilder::new("RuntimeEnum", vec![], enm.into()).build(&mut types);

    insta::assert_snapshot!(
        Typescript::default().export(&types).unwrap(),
        @r#"
// This file has been generated by Specta. Do not edit this file manually.
export type RuntimeEnum = ({ A: boolean }) & { B?: never } | ({ B: number }) & { A?: never };
"#
    );
}
