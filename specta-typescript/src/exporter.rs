...
impl fmt::Debug for FrameworkMapFn {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "FrameworkMapFn({:p})", self.0)
    }
}
...
pub struct Exporter {
    ...
    pub(crate) framework_map: Option<FrameworkMapFn>,
...
impl Exporter {
    ...
    /// Configure a framework-specific transform over exported [`DataType`] values.
    ///
    /// This hook is applied recursively by the public primitives API, allowing Specta-based
    ...
    pub fn framework_map(
        ...
        self.framework_map = Some(FrameworkMapFn(Arc::new(move |dt| {
