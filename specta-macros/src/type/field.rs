...
// Construct a field.
pub fn construct_field(crate_ref: &TokenStream, generics: &Generics, attrs: FieldAttr, field_ty: &Type) -> TokenStream {
    ...
    let generics = generics.params.iter().filter_map(|p| match p {
        ...
        GenericParam::Type(p) => {
            ...
            // TODO: Fix datatpe
            quote!((std::borrow::Cow::Borrowed(#ident_str).into(), #crate_ref::DataType::Any)).into()
        }
    });
